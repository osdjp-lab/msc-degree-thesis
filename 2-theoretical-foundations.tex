...
\begin{itemize}
    ...
    \item \Gls{ma} \cite{whittle} - expresses the current value as a linear combination of $p$ historical error terms. \Glsxtrshort{ma}$(q)$ denotes an \glsxtrlong{ma} model of order $q$. The model is mathematically defined as follows:
        ...
        \begin{itemize}
            ...
            \item $\varepsilon_{t}, \varepsilon_{t-i}$ are the the white noise error terms at time $t$ and $t-i$.
        \end{itemize}
...
The following are a few important mulitvariate models:

\begin{itemize}
    \item \Gls{var} \cite{sims} - is a system of regressions where each variable is modeled as a function of its own lags and the lags of the other variables in the system. A $p$ order \gls{var} is denoted as \gls{var}$(p)$ \cite{deep-learning}. The model can be described as follows:
...
\begin{figure}
    \includegraphics[width=0.5\textwidth]{images/model-diagrams/neural-network.png}
    \centering
...
The primary tunable parameters of a \gls{ann} are:
\begin{itemize}
    \item The number of input, hidden and output nodes.
...
\begin{figure}
    \includegraphics[width=0.5\textwidth]{images/model-diagrams/svm-margin.png}
    \centering
...
\begin{itemize}
    ...
    \item Minimum number of samples per leaf.
\end{itemize}
