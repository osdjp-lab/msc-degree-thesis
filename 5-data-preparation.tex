...
In this paper, data preparation for use in \gls{ai} model creation consists of two steps: acquisition and preprocessing. Acquisition involves the sourcing of suitable data from open access repositories while preprocessing involves the cleaning and preparation of the data conducted in order to increase the accuracy of predictions as well as improve the efficiency of model training.
...
% 
% Include common steps which cannot be ommited carried out before any other preprocessing proceedures.

The following is a summary of all preprocessing steps applied and evaluated in this research. Not all steps are required in the data preparation pipeline for every models however depending on the dataset they may improve performance and accuracy. The steps that were applied in all cases were the handling of missing values, feature creation, feature selection, decorrelation and data split. For the remaining various combinations of log transformation, differencing and scaling were applied to the dataset depending on the target model.
...
% 
% These steps are not required in the data preparation pipeline for use with the \gls{ai} models however they may provide improvements in their performance or accuracy depending on the dataset and model used.

\subsubsection{Log transformation}\label{Log transformation}

In this step a natural logarithm transformation of the data was applied in order to stabilize the variance and prevent \gls{heteroscedasticity} \cite{nn-vs-chaotic,robust-regression,gbp-usd-forecasting}.
...
% 
% Include common steps which cannot be ommited carried out after other optional preprocessing proceedures.

\subsubsection{Feature creation}\label{Feature creation}

For this step a single target variable the \gls{usd} \gls{eer} was chosen. Time lagged versions of each feature variable including the target variable upto 12 months were created for each preprocessing variation \cite{rf-svr,gold}. An example of the shifting procedure for the \gls{usd} exchange rate is shown in figure \ref{fig:feature-creation}.
...
In the next step a Pearson correlation matrix was constructed for all variables to assess the linear relationships among them. Features exhibiting weak correlation $\le 0.1$ with the target were removed first, as they contribute noise without predictive signal. Next feature variables that had a high mutual correlation $\ge 0.9$ with at least one other feature were removed. This was done to mitigate the risk of \gls{multicollinearity}, which can lead to the occurrence of amplifying or distorting effects during model training \cite{genetic}. As a result of this analysis, the initial feature set for each dataset processing variation was significantly reduced.

\subsubsection{Data split}\label{Data split}

For the purpose of model training and forecasting accuracy evaluation, each dataset was split into two sequential subsets, with a gap between them equal to the length of the time lag used, in order to prevent information leakage from the training set to the test set. The proportion of the split for both parts was as follows:
\begin{itemize}
